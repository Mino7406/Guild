    ...
    public static void main(String[] args) {
        ...
        try {
            JDA jda = JDABuilder.createDefault(token).enableIntents(GatewayIntent.MESSAGE_CONTENT)
                    .addEventListeners(new Monster()).build();
            ...
            jda.updateCommands()
                    .addCommands(Commands.slash("몬스터", "해당 몬스터의 정보를 확인합니다. (⚠️Beta - 현재 9성몹만 검색 가능합니다.) ").addOption(
                            OptionType.STRING, "이름", "검색할 몬스터의 이름을 입력하세요. [ ※ 별명이나 줄임말로도 검색 가능합니다. ]", true, true))
                    .queue();
